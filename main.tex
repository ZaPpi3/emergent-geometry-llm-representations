...
circle a first PCA plot suggests. We report all five results, the bugs found
and fixed along the way, and why we believe even the negative ones are
...
we trust them together.

\appendix
\section{Original planning notes}
\label{app:notes}
The investigation began from a short planning document (\texttt{NOTES.md}),
reproduced in the project repository, which framed the goal as testing
whether ``a continuous, geometric concept-manifold spontaneously
reconstruct[s] itself out of abstract machine data'' via mutual-information/
cosine-similarity graphs and graph Laplacian diagonalization. All code
referenced in this paper is included under \texttt{code/}, and the raw
intermediate data (activation vectors, similarity graphs, eigenmode
projections) is included under \texttt{data/}, at each project phase.
